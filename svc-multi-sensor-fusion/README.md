# svc-multi-sensor-fusion

## 服务说明
- 服务名：`svc-multi-sensor-fusion`
- 服务作用：模拟多传感器融合分析任务的计算负载服务
- 宿主机建议映射端口：`6009`
...
### curl
```bash
curl -X POST "http://127.0.0.1:6009/infer_once" \
...
### docker build
```bash
docker build -t svc-multi-sensor-fusion:latest .
```

### docker run
```bash
docker run -d --name svc-multi-sensor-fusion -p 6009:6000 svc-multi-sensor-fusion:latest
```
