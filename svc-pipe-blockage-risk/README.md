# svc-pipe-blockage-risk

## 服务说明
- 服务名：`svc-pipe-blockage-risk`
- 服务作用：模拟管道堵塞风险识别任务的计算负载服务
- 宿主机建议映射端口：`6003`
...
### curl
```bash
curl -X POST "http://127.0.0.1:6003/infer_once" \
...
### docker build
```bash
docker build -t svc-pipe-blockage-risk:latest .
```

### docker run
```bash
docker run -d --name svc-pipe-blockage-risk -p 6003:6000 svc-pipe-blockage-risk:latest
```
