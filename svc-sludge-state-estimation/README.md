# svc-sludge-state-estimation

## 服务说明
- 服务名：`svc-sludge-state-estimation`
- 服务作用：模拟污泥状态估计任务的计算负载服务
- 宿主机建议映射端口：`6007`
...
### curl
```bash
curl -X POST "http://127.0.0.1:6007/infer_once" \
...
### docker build
```bash
docker build -t svc-sludge-state-estimation:latest .
```

### docker run
```bash
docker run -d --name svc-sludge-state-estimation -p 6007:6000 svc-sludge-state-estimation:latest
```
