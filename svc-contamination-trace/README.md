# svc-contamination-trace

## 服务说明
- 服务名：`svc-contamination-trace`
- 服务作用：模拟污染源追踪任务的计算负载服务
- 宿主机建议映射端口：`6008`
...
### curl
```bash
curl -X POST "http://127.0.0.1:6008/infer_once" \
...
### docker build
```bash
docker build -t svc-contamination-trace:latest .
```

### docker run
```bash
docker run -d --name svc-contamination-trace -p 6008:6000 svc-contamination-trace:latest
```
