...
## 服务说明
- 服务名：`svc-cod-prediction`
- 服务作用：模拟 COD 指标预测任务的计算负载服务
- 宿主机建议映射端口：`6005`
...
### curl
```bash
curl -X POST "http://127.0.0.1:6005/infer_once" \
...
### docker build
```bash
docker build -t svc-cod-prediction:latest .
```

### docker run
```bash
docker run -d --name svc-cod-prediction -p 6005:6000 svc-cod-prediction:latest
```
