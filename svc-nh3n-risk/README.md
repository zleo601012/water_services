...
### curl
```bash
curl -X POST "http://127.0.0.1:6001/infer_once" \
...
### docker build
```bash
docker build -t svc-nh3n-risk:latest .
```

### docker run
```bash
docker run -d --name svc-nh3n-risk -p 6001:6000 svc-nh3n-risk:latest
```
