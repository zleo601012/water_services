# svc-pump-load-forecast

## 服务说明
- 服务名：`svc-pump-load-forecast`
- 服务作用：模拟泵站负载预测任务的计算负载服务
- 宿主机建议映射端口：`6011`
...
### curl
```bash
curl -X POST "http://127.0.0.1:6011/infer_once" \
...
### docker build
```bash
docker build -t svc-pump-load-forecast:latest .
```

### docker run
```bash
docker run -d --name svc-pump-load-forecast -p 6011:6000 svc-pump-load-forecast:latest
```
