# svc-water-quality-anomaly

## 服务说明
- 服务名：`svc-water-quality-anomaly`
- 服务作用：模拟水质异常检测任务的计算负载服务
- 宿主机建议映射端口：`6004`
...
### curl
```bash
curl -X POST "http://127.0.0.1:6004/infer_once" \
...
### docker build
```bash
docker build -t svc-water-quality-anomaly:latest .
```

### docker run
```bash
docker run -d --name svc-water-quality-anomaly -p 6004:6000 svc-water-quality-anomaly:latest
```
