# svc-turbidity-prediction

## 服务说明
- 服务名：`svc-turbidity-prediction`
- 服务作用：模拟浊度预测任务的计算负载服务
- 宿主机建议映射端口：`6006`
...
### curl
```bash
curl -X POST "http://127.0.0.1:6006/infer_once" \
...
### docker build
```bash
docker build -t svc-turbidity-prediction:latest .
```

### docker run
```bash
docker run -d --name svc-turbidity-prediction -p 6006:6000 svc-turbidity-prediction:latest
```
