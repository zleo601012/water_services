...
### curl
```bash
curl -X POST "http://127.0.0.1:6002/infer_once" \
...
### docker build
```bash
docker build -t svc-node-health:latest .
```

### docker run
```bash
docker run -d --name svc-node-health -p 6002:6000 svc-node-health:latest
```
